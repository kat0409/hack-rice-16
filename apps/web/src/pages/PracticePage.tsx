...
import type { QuestionsContent } from '@/lib/api'
...
export function PracticePage() {
  ...
  const { artifact, loading, error } = useArtifact<QuestionsContent>(step?.id, 'QUESTIONS')
  ...
  const choose = (optionIndex: number) => {
    ...
    setScore((prev) => ({
      correct: prev.correct + (optionIndex === question.correct_index ? 1 : 0),
  ...
  const next = () => {
    setSelected(null)
    if (isLast) setScore({ correct: 0, answered: 0 })
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {loading && <p className="text-sm text-ink-soft">Writing questions from your notes…</p>}
          ...
          <div className="rounded-2xl border-2 border-ink/10 bg-paper p-6 shadow-chunky">
            ...
            {selected !== null && (
              <div className="mt-5 flex flex-col gap-3 border-t-2 border-dashed border-ink/10 pt-4">
                ...
                {question.citation_labels[0] && (
                  <p className="text-xs italic text-ink-soft/60">{question.citation_labels.join(' · ')}</p>
                )}
