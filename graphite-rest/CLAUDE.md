...
Add dependencies with `uv add <package>` — never hand-edit `pyproject.toml`'s dependency list or `uv.lock`. `uv sync` alone stays CPU-only (~600MB of deps for local voice, no torch/onnxruntime-gpu); `uv sync --extra gpu` additionally installs the CUDA runtime libs (`nvidia-cublas-cu12`, `nvidia-cudnn-cu12`, ~1.3GB) that let faster-whisper actually run on a GPU instead of just detecting one.
...
Copy repo-root `.env.example` to `.env` before running against a real database (`DATABASE_URL`, `GEMINI_API_KEY`, etc. — see `graphite/config.py` for every field and its default; voice needs none of these, it runs entirely locally). Without a `.env`/reachable Postgres, the app **still boots**: `/health` reports `{"status":"degraded","database":"down"}` and DB-backed routes return `503 DB_UNAVAILABLE` instead of crashing. That's intentional graceful-degradation behavior (useful in sandboxes with no Docker access) — don't treat it as a bug to silently "fix" by making startup fail hard.
...
- `routers/` — one thin `APIRouter` per resource:
  ...
  - `audio.py` — `POST /api/v1/courses/{course_id}/audio-transcriptions`: multipart audio → `services/local_voice.py` (faster-whisper, runs in a threadpool) → transcript text back to the caller (design-doc.md §10.4 "voice input" — the typed/transcribed text must stay editable client-side, this endpoint's only job is producing it). `persist_as_document=true` (query param) additionally saves the transcript as a `documents` row so it can flow through the same ingestion status pipeline as an uploaded `.txt` — this is an interpretation beyond the literal spec, flagged in the code, not a spec requirement.
  - `tutor.py` — `POST /api/v1/courses/{course_id}/tutor/turns`: a question (+ recent `history` sent by the client; stateless, no table) → `graphite.tutor.answer_question` → a grounded, cited, speech-friendly answer, then TTS cached by content hash under `data/narration-cache/` like narration. Voice is optional: a TTS failure returns the text with `audio_id: null` and `audio_error`. `GET /api/v1/tutor-audio/{audio_id}` streams it; the id is validated as 64 hex chars before touching the filesystem.
- `services/local_voice.py` — local speech-to-text (faster-whisper, downloaded from Hugging Face) and text-to-speech (Kokoro/`kokoro-onnx`, downloaded from its GitHub release) — no API key, no network once models are cached under `Settings.voice_model_dir_path`. `stt_device`/`tts_device` default to `"auto"`: a CUDA GPU is tried first and silently falls back to CPU (`int8`) if the runtime libs (cuBLAS/cuDNN) aren't installed — see the `[gpu]` optional dependency group in `pyproject.toml` to enable them. Model load failure → `503 TRANSCRIPTION_UNAVAILABLE`/`NARRATION_UNAVAILABLE`, never a crash.
...
- **Narration** — `POST /study-artifacts/{id}/narration` voices a SUMMARY via local TTS (`services/local_voice.py::synthesize_speech`), cached under `data/narration-cache/` as `{hash}.wav`; `GET /narrations/{id}/audio` streams it. Cache files from before the switch to local voice are `{hash}.mp3`; `narration.py::audio_path()` serves either, so old Replay links still work.
